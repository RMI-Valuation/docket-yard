...
| `route_class` | `home`, `sheet`, `record`, `document`, `party`, `parties`, `search` (`/search` and `/suggest`), `week`, `stats`, `feed`, `json`, `sitemap` (and robots), `data` (`/data`, `/api`, `/llms.txt`, `/openapi.json`), `trust` (about/contribute/coverage/methodology/corrections/privacy), `subscribe` (and the token and SES paths), `static`, `other` — `ROUTE_CLASSES` in `store/traffic.py` | the page kind, never the page: no docket, no party, no record id |

...
| `last_event` — newest filing or decision observation (scoped to those two types on purpose: environmental comments also write to the ledger from 2026-08-31, and an unscoped canary would let them mask a filings or decisions parser failure) | Captures arrive but nothing new is parsed from them: the parser broke, or the Board is genuinely quiet | 6 days: the Board serves something most business days; a long weekend must not page |
| `last_enviro_capture` — newest asserted forward capture of the environmental-comment table | The comment poll is dead, or refused, **or its parser broke** — an asserted capture means the response was parsed and every criterion positively verified, so a renamed criterion or a markup change fails here rather than waiting for the event window | 3 hours, the same as the other tables and deliberately not relaxed: a genuinely quiet week still produces an asserted capture, because the quiet-week proof asks a wider window and that answer asserts |
| `last_enviro_event` — newest environmental-comment observation | Comments parse but none is recorded: the fold or the ledger write broke | **21 days**, measured rather than borrowed. The table runs at ~4 comments a week against the record's ~60, and its longest legitimate silence across five years is **14 days** (2026; 13 in 2024 and 2025, 10 in 2019, 9 in 2023) against a median gap of 1–2 days. The filings threshold of 6 days would page several times a year for nothing |

...
| `docs/adr/` | **0001-0015 all Accepted** (0002-0008 and 0010-0015 carry Validation sections); **0016 Accepted** 2026-08-28 (reviewer identity); **0017 and 0018 Accepted** 2026-09-01 — what citation edges ship at (89.3% projected, 98.0% precision) and the five assertion families that hold them (0018's addenda Accepted 2026-09-13: a retraction retires the key's readings too, dated, each with a retirement row; and 2026-09-14: a six-digit number whose stripped form is the document's own docket keys as that docket, the finding carrying its key — shipped in v2026.09.24 as finder 2026-09-14b, rank v7); eight items owed at the migration, in 0018 § Owed; **0019 and 0020 Accepted** 2026-09-02 — telemetry and detection, and maintenance mode; both addenda to 0012, which now points at them; **0021 and 0022 Accepted** 2026-09-02 — the OCR text grain, and where the record's text lives (a third addendum to 0012). The migration they authorise is scoped in `docs/ocr-migration.md` § Migration A; **0023 Accepted** 2026-09-03 — the render and the engine's NAME in a quoted date's key, and the rebuild that reaches what an ALTER cannot (migration 0019). Its pick rule was decided 2026-09-03 (addendum): compare values — publish only when every live reading agrees; **0024 Accepted** 2026-09-10 — text for new material is extracted on the instance, in a container holding no store and no key; scoped by document, bounded by a cap and never a look-back window. Accepted and withdrawn 2026-09-05 on the critic's findings, rewritten, accepted; migrations 0023–0024 apply it, and `/security-review` is owed before it first ships; **0025 Accepted** 2026-09-10 — derivation runs on leased pages on the operator's machines, a pass is a reading key, and the fleet is watched the way production is (addendum 2026-09-19, proposals 1–4 **Accepted** 2026-09-19: a broker places the readers and never learns what a pass is, preemption is the yield the record already has, something must resubmit, the coordinator is backed up while readers stay disposable, and a reader does not also coordinate. Proposals 5–6 — a blob miss is a fetch, and what decision 6 means — **Accepted** 2026-09-19: the credential carries `GetObject` + `ListBucket` on the blob prefix so a 404 is the document's fault and a 403 the environment's. **The decision is accepted, its first draft is not** — two reviews returned it, and a rebuild on the project's own signed GET, streaming, with structural error classes and the worker half in the same change is owed before anything runs); **0026 Accepted** 2026-09-12 — a citation reading names the text it read: a `text_id` FK and a `text_ref` vocabulary on `citation_reading`, staleness detected over `document_text_display`, the live key deliberately NOT widened (on a measured price, not a deferral), `source_location` as `{page, spans}` with a per-span raw, `superseded_at` on a forward-only trigger, and a rebuild plus a re-load behind the wall. Two critic passes; the offsets that started it are D4 and recover 12 of 506 rows |
| `docs/stb-data-source.md` | Endpoint mechanics, its silent-failure traps, and every measurement taken |
| `docs/capability-map.md` | The 30 capabilities. A menu, not a roadmap. |

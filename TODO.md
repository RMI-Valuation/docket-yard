...
- **The six-digit own-docket rule** (346 findings): ADR 0018 addendum of 2026-09-14 ACCEPTED by him
  (items 1-12, five critic passes). Build as v7 on a branch — WAITS FOR HIS GO: keys.py rule,
  verify_spans/load with own, per-span check, anchor on key + rule version, successor shape, tools,
  cards for both channels, rehearsal in prod image counting the 61 own-key citations, PR, re-load

...
def test_a_letter_page_renders_at_200_and_a_plan_sheet_fits_the_long_edge():
    ...
    assert c.RENDER_PROFILE == "200-max2576" and "/" not in c.RENDER_PROFILE
...
def test_collect_writes_what_the_loader_takes_and_a_cut_or_errored_page_is_a_failure(tmp_path):
    ...
    assert (doc["method"], doc["render_profile"]) == ("claude-sonnet-5", "200-max2576")
    ...
    con.close()
    assert rows == [(1, "ocr", "claude-sonnet-5", "200-max2576", "a faint fax page read well")]
